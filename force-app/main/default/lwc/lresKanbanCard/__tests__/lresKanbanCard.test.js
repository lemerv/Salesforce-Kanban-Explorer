...
import { createDataTransfer, flushPromises } from '../../lresTestUtils/lresTestUtils';
...
describe('c-lres-kanban-card', () => {
    ...
    it('dispatches drag start with payload and sets dataTransfer', async () => {
        ...
        const dragEvent = new CustomEvent('dragstart', { bubbles: true, cancelable: true });
        ...
        expect(handler).toHaveBeenCalledWith(
            expect.objectContaining({ detail: { recordId: '001', columnKey: 'col1' } })
        );
    });
    ...
    it('dispatches drag end and title click events', () => {
        ...
        const titleLink = element.shadowRoot.querySelector('.kanban-card_title-link');
    ...
    describe('Template Rendering', () => {
        ...
        it('renders card with icon when titleIcon is provided', () => {
            const element = buildComponent({
                card: { id: '001', title: 'Test Deal', titleIcon: 'standard:account', details: [] }
            });
...
//            expect(icon.getAttribute('icon-name')).toBe('standard:account'); // Note: mock doesn't handle attributes
        ...
        it('renders details section when card has details', () => {
            const details = [
                { apiName: 'Amount', label: 'Amount', value: '$1000', dataType: 'currency' },
                { apiName: 'Stage', label: 'Stage', value: 'Prospecting', dataType: 'string' }
            ];
            const element = buildComponent({
                card: { id: '001', title: 'Test Deal', details }
            });

            const detailsContainer = element.shadowRoot.querySelector('.kanban-card_details');
            expect(detailsContainer).toBeTruthy();

            const fieldComponents = detailsContainer.querySelectorAll('c-lres-kanban-card-field');
        ...
        it('renders a new-tab icon that does not trigger the modal click event', () => {
            ...
            const externalLink = element.shadowRoot.querySelector('.kanban-card_external-link');
