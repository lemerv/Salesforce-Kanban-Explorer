...
describe('c-lres-kanban-board-actions', () => {
    ...
    it('dispatches search events from lightning input', () => {
        ...
        element.addEventListener('searchkeyup', handler);

        const input = element.shadowRoot.querySelector('lightning-input');
        input.value = 'abc';
        input.dispatchEvent(new CustomEvent('change', { detail: { value: 'abc' } }));
        input.dispatchEvent(new CustomEvent('keyup', { detail: { value: 'abc' } }));

        expect(handler).toHaveBeenCalledTimes(2);
        expect(handler.mock.calls[0][0].detail.value).toBe('abc');
        expect(handler.mock.calls[1][0].detail.value).toBe('abc');
    });
    ...
    it('dispatches sort option and direction events', () => {
        ...
        option.dispatchEvent(new CustomEvent('change', { detail: { value: 'Name' } }));

        const direction = element.shadowRoot.querySelector('lightning-button-icon.board-actions_direction');
    ...
    it('dispatches manual refresh and clear filter events', () => {
        ...
        const clearButton = element.shadowRoot.querySelector('.board-actions_clear');
        clearButton.click();

        const refreshButton = element.shadowRoot.querySelector('.board-actions_refresh');
