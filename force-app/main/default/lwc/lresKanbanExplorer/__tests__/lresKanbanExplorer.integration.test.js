...
import { flushPromises, settleComponent, buildWireRecordsFromRaw } from '../../lresTestUtils/lresTestUtils';
...
describe('c-lres-kanban-explorer integration', () => {
    ...
    const baseRecords = buildWireRecordsFromRaw(
        [
            { id: '001', fields: { Id: '001', Status__c: 'Open', Name: 'Deal 1', Stage__c: 'Prospecting' } },
            { id: '002', fields: { Id: '002', Status__c: 'Closed', Name: 'Deal 2', Stage__c: 'Closed Won' } }
        ],
        'Opportunity'
    );
    ...
    it('supports end-to-end drag and drop workflow with state sync', async () => {
        ...
        expect(container.columns.find((col) => col.key === 'Open').records).toHaveLength(1);
        ...
        expect(updateRecord).toHaveBeenCalledWith({ fields: { Id: '001', Status__c: 'Closed' } });
    ...
    it('updates when parent selection changes and applies new Apex data', async () => {
        const updatedRecords = buildWireRecordsFromRaw(
            [
                { id: '003', fields: { Id: '003', Status__c: 'Closed', Name: 'Deal 3', Stage__c: 'Closed Won' } }
            ],
            'Opportunity'
        );
        ...
        const parentSelector = element.shadowRoot.querySelector('c-lres-kanban-parent-selector');
        ...
        expect(fetchRelatedCardRecords.mock.calls.length).toBeGreaterThan(beforeCalls);
        const container = element.shadowRoot.querySelector('c-lres-kanban-board-container');
        expect(container.columns.find((col) => col.key === 'Closed').records).toHaveLength(1);
    });

    it('supports multi-parent selection with debounced Apex refresh', async () => {
        const multiRecords = buildWireRecordsFromRaw(
            [
                { id: '010', fields: { Id: '010', Status__c: 'Open', Name: 'Multi Deal', Stage__c: 'Prospecting' } }
            ],
            'Opportunity'
        );
