...
export const TIME_TOKEN_CHARS = new Set(['H', 'h', 'K', 'k', 'm', 's', 'S', 'a', 'y', 'M', 'd', 'E', 'z']);

export function formatValueWithPattern(value, { dateOnly = false, pattern, patternTokenCache, locale = DEFAULT_LOCALE, timeZone = DEFAULT_TIME_ZONE } = {}) {
    const parts = buildDateFormatParts(value, { dateOnly, locale, timeZone });
    if (!parts) {
        return null;
    }
    const formatted = applyDateTimePattern(parts, pattern, { dateOnly, patternTokenCache });
    if (!formatted) {
        return null;
    }
    if (dateOnly) {
...
export function buildDateFormatParts(value, { dateOnly = false, locale = DEFAULT_LOCALE, timeZone = DEFAULT_TIME_ZONE } = {}) {
    ...
    if (dateOnly && typeof value === 'string') {
        ...
        if (match) {
            ...
            return {
                ...
                millisecond: '000',
                amPm: '',
                monthShort: new Intl.DateTimeFormat(locale, { month: 'short', timeZone: 'UTC' }).format(baseDate),
                monthLong: new Intl.DateTimeFormat(locale, { month: 'long', timeZone: 'UTC' }).format(baseDate),
                weekdayShort: new Intl.DateTimeFormat(locale, { weekday: 'short', timeZone: 'UTC' }).format(baseDate),
                weekdayLong: new Intl.DateTimeFormat(locale, { weekday: 'long', timeZone: 'UTC' }).format(baseDate),
                timeZoneNameShort: '',
                timeZoneNameLong: ''
            };
        }
    ...
    const shortZone = timeZoneFormatter.formatToParts(dateObj).find((part) => part.type === 'timeZoneName');
    const longZone = longTimeZoneFormatter.formatToParts(dateObj).find((part) => part.type === 'timeZoneName');
    ...
    return {
        ...
        monthShort: new Intl.DateTimeFormat(locale, { month: 'short', timeZone }).format(dateObj),
        monthLong: new Intl.DateTimeFormat(locale, { month: 'long', timeZone }).format(dateObj),
        weekdayShort: new Intl.DateTimeFormat(locale, { weekday: 'short', timeZone }).format(dateObj),
        weekdayLong: new Intl.DateTimeFormat(locale, { weekday: 'long', timeZone }).format(dateObj),
        timeZoneNameShort: shortZone?.value || '',
        timeZoneNameLong: longZone?.value || ''
    };
}

export function applyDateTimePattern(parts, pattern, { dateOnly = false, patternTokenCache } = {}) {
    if (!pattern || !parts) {
        return null;
    }
    ...
    for (let i = 0; i < tokens.length; i += 1) {
        ...
        if (token.type === 'literal') {
            const futureOutput = hasFutureTokenOutput(tokens, i + 1, parts, { dateOnly });
            const shouldInclude = futureOutput || hasOutput;
            if (!shouldInclude) {
                continue;
            }
            if (!futureOutput && dateOnly && CONNECTOR_LITERAL_PATTERN.test(token.value)) {
                continue;
            }
            result += token.value;
            continue;
...
function hasFutureTokenOutput(tokens, startIndex, parts, { dateOnly = false } = {}) {
