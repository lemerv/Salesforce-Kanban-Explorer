...
describe('filterUtils.buildFilterDefinitions', () => {
    ...
    const sortFilterOptions = (options) => options.sort((a, b) => a.label.localeCompare(b.label));
    const getFilterButtonClass = (id, hasSelection) => `${id}-${hasSelection ? 'on' : 'off'}`;

    it('builds definitions from blueprints and records', () => {
        const logDebug = jest.fn();
        const { definitions, activeFilterMenuId, shouldCloseMenus } = buildFilterDefinitions({
            records,
            blueprints,
            existingDefinitions: [],
    ...
    it('preserves previous selections and open state', () => {
        ...
        expect(statusDef.options.find((opt) => opt.value === 'New').selected).toBe(true);
